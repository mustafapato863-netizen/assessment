# Multi-stage Dockerfile for AssessFlow Backend API
...
RUN pnpm --filter @assessflow/contracts build
RUN pnpm --filter @assessflow/database build
RUN pnpm --filter @assessflow/api build
